...
export default function RegexTester() {
  ...
  return (
    <ToolLayout
      title="Regex Tester"
      description="Test regular expressions with real-time matching, highlighting, and capture group support."
      toolId="regex-tester"
    >
      {/* Pattern */}
      <div style={{ marginBottom: 16 }}>
        <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 8 }}>Regular Expression</label>
      ...
      {/* Test String */}
      <div style={{ marginBottom: 16 }}>
        <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 8 }}>Test String</label>
        <textarea value={testStr} onChange={e => setTestStr(e.target.value)}
          placeholder="Enter your test string here..." style={{ minHeight: 150 }} />
      </div>

      {/* Highlighted output */}
      {typeof highlightedText !== 'string' && highlightedText.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 8 }}>Highlighted Matches</label>
          <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {matches.map((m, i) => (
              <div key={i} style={{
                ...
                <span style={{ color: 'var(--text-secondary)' }}> at index {m.index}</span>
                {m.groups.length > 0 && (
                  <span style={{ color: 'var(--accent-emerald)' }}> groups: [{m.groups.join(', ')}]</span>
                )}
              </div>
            ))}
          </div>
      ...
      <div style={{ marginTop: 30, paddingTop: 20, borderTop: '1px solid var(--border-color)' }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 12 }}>About Regular Expressions</h2>
