  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div>
          <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 8 }}>Original Text</label>
          <textarea value={text1} onChange={e => { setText1(e.target.value); setShowDiff(false); }}
            placeholder="Paste original text here..." style={{ minHeight: 200 }} />
        </div>
        <div>
          <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 8 }}>Modified Text</label>
          <textarea value={text2} onChange={e => { setText2(e.target.value); setShowDiff(false); }}
            placeholder="Paste modified text here..." style={{ minHeight: 200 }} />
        </div>
      </div>

      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button onClick={() => setShowDiff(true)} className="btn btn-primary">Compare</button>
        <button onClick={() => { setText1(''); setText2(''); setShowDiff(false); }} className="btn btn-secondary">Clear</button>
      </div>

      {showDiff && diff.length > 0 && (
        <>
          <div style={{ display: 'flex', gap: 16, marginBottom: 16, fontSize: 13 }}>
            <span style={{ color: 'var(--accent-emerald)' }}>+ {stats.added} added</span>
            <span style={{ color: 'var(--accent-rose)' }}>- {stats.removed} removed</span>
            <span style={{ color: 'var(--text-secondary)' }}>{stats.same} unchanged</span>
          </div>
      ...
      <div style={{ marginTop: 30, paddingTop: 20, borderTop: '1px solid var(--border-color)' }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 12 }}>About Text Diff</h2>
