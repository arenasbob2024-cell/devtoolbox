...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'openapi-to-typescript');
  const url = `https://viadreams.cc/${lang}/tools/openapi-to-typescript`;
  return {
    ...
    alternates: { canonical: url, languages: { ...Object.fromEntries(i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/openapi-to-typescript`])), 'x-default': `https://viadreams.cc/en/tools/openapi-to-typescript` } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  const { lang: rawLang } = await params;
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return <ToolSeoServer toolId="openapi-to-typescript" lang={lang}>{children}</ToolSeoServer>;
}
