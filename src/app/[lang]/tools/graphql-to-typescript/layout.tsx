...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'graphql-to-typescript');
  const url = `https://viadreams.cc/${lang}/tools/graphql-to-typescript/`;
  return {
    title: t?.pageTitle,
    ...
    alternates: { canonical: url, languages: {
        ...Object.fromEntries(
          i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/graphql-to-typescript/`])
        ),
        'x-default': `https://viadreams.cc/en/tools/graphql-to-typescript/`,
      } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return (
    <ToolSeoServer toolId="graphql-to-typescript" lang={lang}>
      {children}
    </ToolSeoServer>
  );
}
