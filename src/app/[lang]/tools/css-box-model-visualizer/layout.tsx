...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'css-box-model-visualizer');
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...Object.fromEntries(
          i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/css-box-model-visualizer/`])
        ),
        'x-default': `https://viadreams.cc/en/tools/css-box-model-visualizer/`,
      },
    },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return (
    <ToolSeoServer toolId="css-box-model-visualizer" lang={lang}>
      {children}
    </ToolSeoServer>
  );
}
