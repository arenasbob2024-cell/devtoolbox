...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'color-name-to-hex');
  const url = `https://viadreams.cc/${lang}/tools/color-name-to-hex/`;
  return {
    title: t?.pageTitle,
    ...
    alternates: { canonical: url, languages: {
        ...Object.fromEntries(
          i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/color-name-to-hex/`])
        ),
        'x-default': `https://viadreams.cc/en/tools/color-name-to-hex/`,
      } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return (
    <ToolSeoServer toolId="color-name-to-hex" lang={lang}>
      {children}
    </ToolSeoServer>
  );
}
