...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'html-to-pug');
  const url = `https://viadreams.cc/${lang}/tools/html-to-pug/`;
  return {
    ...
    alternates: { canonical: url, languages: { ...Object.fromEntries(i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/html-to-pug/`])), 'x-default': `https://viadreams.cc/en/tools/html-to-pug/` } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  const { lang: rawLang } = await params;
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return <ToolSeoServer toolId="html-to-pug" lang={lang}>{children}</ToolSeoServer>;
}
