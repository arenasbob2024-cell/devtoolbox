...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = dict.tools['env-editor'];
  const url = `https://viadreams.cc/${lang}/tools/env-editor`;
  return {
    title: t.pageTitle,
    ...
    alternates: { canonical: url, languages: { ...Object.fromEntries(i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/env-editor`])), 'x-default': `https://viadreams.cc/en/tools/env-editor` } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  const { lang: rawLang } = await params;
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return <ToolSeoServer toolId="env-editor" lang={lang}>{children}</ToolSeoServer>;
}