...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'ascii-text-converter');
  const url = `https://viadreams.cc/${lang}/tools/ascii-text-converter/`;
  return {
    title: t?.pageTitle,
    ...
    alternates: { canonical: url, languages: {
        ...Object.fromEntries(
          i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/ascii-text-converter/`])
        ),
        'x-default': `https://viadreams.cc/en/tools/ascii-text-converter/`,
      } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return (
    <ToolSeoServer toolId="ascii-text-converter" lang={lang}>
      {children}
    </ToolSeoServer>
  );
}
