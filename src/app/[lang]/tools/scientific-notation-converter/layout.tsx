...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'scientific-notation-converter');
  const url = `https://viadreams.cc/${lang}/tools/scientific-notation-converter`;
  return {
    title: t?.pageTitle,
    ...
    alternates: { canonical: url, languages: {
        ...Object.fromEntries(
          i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/scientific-notation-converter`])
        ),
        'x-default': `https://viadreams.cc/en/tools/scientific-notation-converter`,
      } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return (
    <ToolSeoServer toolId="scientific-notation-converter" lang={lang}>
      {children}
    </ToolSeoServer>
  );
}
