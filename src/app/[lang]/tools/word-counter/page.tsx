...
export default function WordCounter() {
  const { dict } = useLang();
  const t = dict.tools['word-counter'];
  ...
  const statCards = [
    { label: dict.common.words, value: stats.words, color: 'var(--accent-blue)' },
    { label: dict.common.characters, value: stats.chars, color: 'var(--accent-purple)' },
    { label: t.noSpaces, value: stats.charsNoSpaces, color: 'var(--accent-emerald)' },
    ...
    { label: t.lines, value: stats.lines, color: 'var(--text-secondary)' },
  ];
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        ⏱ {t.readingTime}: <strong style={{ color: 'var(--accent-blue)' }}>{stats.readingTime}</strong> {t.readingTimeNote}
      </div>
      ...
      {topWordsData.length > 0 && (
        <div style={{ marginTop: 16 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 8 }}>{t.topWords}</h3>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {topWordsData.map(([word, count]) => (
              <span key={word} style={{
