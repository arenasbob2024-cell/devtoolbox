...
export default function SqlFormatter() {
  const { dict } = useLang();
  const t = dict.tools['sql-formatter'];
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <button onClick={() => setOutput(formatSql(input))} className="btn btn-primary">{t.formatSql}</button>
        <button onClick={() => setOutput(minifySql(input))} className="btn btn-secondary">{dict.common.minify}</button>
        <button onClick={() => setInput(sampleSql)} className="btn btn-secondary">{dict.common.loadSample}</button>
        <button onClick={() => { setInput(''); setOutput(''); }} className="btn btn-secondary">{dict.common.clear}</button>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div>
          <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 8 }}>{t.inputSql}</label>
          <textarea value={input} onChange={e => setInput(e.target.value)}
            placeholder={t.placeholder} style={{ minHeight: 350 }} />
        </div>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <label style={{ fontSize: 13, fontWeight: 600 }}>{t.formattedOutput}</label>
