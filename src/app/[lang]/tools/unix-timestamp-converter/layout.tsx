...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'unix-timestamp-converter');
  const url = `https://viadreams.cc/${lang}/tools/unix-timestamp-converter`;
  return {
    title: t?.pageTitle,
    ...
    alternates: { canonical: url, languages: {
        ...Object.fromEntries(
          i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/unix-timestamp-converter`])
        ),
        'x-default': `https://viadreams.cc/en/tools/unix-timestamp-converter`,
      } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return (
    <ToolSeoServer toolId="unix-timestamp-converter" lang={lang}>
      {children}
    </ToolSeoServer>
  );
}
