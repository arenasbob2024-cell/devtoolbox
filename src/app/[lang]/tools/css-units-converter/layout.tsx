...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'css-units-converter');
  const url = `https://viadreams.cc/${lang}/tools/css-units-converter/`;
  return {
    ...
    alternates: { canonical: url, languages: { ...Object.fromEntries(i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/css-units-converter/`])), 'x-default': `https://viadreams.cc/en/tools/css-units-converter/` } },
  };
}
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  const { lang: rawLang } = await params;
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return <ToolSeoServer toolId="css-units-converter" lang={lang}>{children}</ToolSeoServer>;
}
