...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'json-to-csharp');
  const url = `https://viadreams.cc/${lang}/tools/json-to-csharp`;
  return {
    title: t?.pageTitle,
    ...
    alternates: { canonical: url, languages: {
        ...Object.fromEntries(
          i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/json-to-csharp`])
        ),
        'x-default': `https://viadreams.cc/en/tools/json-to-csharp`,
      } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return (
    <ToolSeoServer toolId="json-to-csharp" lang={lang}>
      {children}
    </ToolSeoServer>
  );
}
