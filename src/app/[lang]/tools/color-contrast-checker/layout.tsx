...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'color-contrast-checker');
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...Object.fromEntries(
          i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/color-contrast-checker/`])
        ),
        'x-default': `https://viadreams.cc/en/tools/color-contrast-checker/`,
      },
    },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return (
    <ToolSeoServer toolId="color-contrast-checker" lang={lang}>
      {children}
    </ToolSeoServer>
  );
}
