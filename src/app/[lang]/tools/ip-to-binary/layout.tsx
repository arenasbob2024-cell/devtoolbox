...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const t = await getToolEntry(lang, 'ip-to-binary');
  const url = `https://viadreams.cc/${lang}/tools/ip-to-binary`;
  return {
    title: t?.pageTitle,
    ...
    alternates: { canonical: url, languages: {
        ...Object.fromEntries(
          i18n.locales.map((l) => [l, `https://viadreams.cc/${l}/tools/ip-to-binary`])
        ),
        'x-default': `https://viadreams.cc/en/tools/ip-to-binary`,
      } },
  };
}
...
export default async function Layout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const lang = (i18n.locales.includes(rawLang as Locale) ? rawLang : i18n.defaultLocale) as Locale;
  return (
    <ToolSeoServer toolId="ip-to-binary" lang={lang}>
      {children}
    </ToolSeoServer>
  );
}
