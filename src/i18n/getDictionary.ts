...
function getDictionaryLoader<T>(
  dictionaries: Partial<Record<Locale, DictionaryLoader<T>>>,
  locale: Locale
) {
...
export const getDictionary = async (locale: Locale): Promise<Dictionary> => {
  const [ui, toolsData] = await Promise.all([
    getDictionaryLoader(uiDictionaries, locale)(),
    getDictionaryLoader(toolsDictionaries, locale)(),
  ]);
...
export const getUIDictionary = async (locale: Locale): Promise<UIDictionary> => {
  return getDictionaryLoader(uiDictionaries, locale)();
};
...
export const getToolEntry = async (
  ...
  const toolsData = await getDictionaryLoader(toolsDictionaries, locale)();
