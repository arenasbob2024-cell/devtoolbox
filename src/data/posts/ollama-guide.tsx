...
const translations: Record<string, Record<string, string>> = {
  ...
  es: {
    title: 'Guia completa de Ollama 2026: ejecutar LLMs localmente, modelos, API y mejores practicas',
    intro: 'Ollama es una herramienta open-source para ejecutar modelos de lenguaje grandes (LLMs) directamente en tu equipo. Si quieres privacidad, cero costes por API u opciones offline, Ollama permite descargar, ejecutar y gestionar modelos como Llama 3, Mistral, Code Llama, Phi-3 y Gemma 2 con comandos simples.',
    tldr: 'Resumen rapido',
    tldrText: 'Instala Ollama en macOS, Linux, Windows o Docker; ejecuta ollama run llama3 para empezar; usa ollama pull para descargar modelos sin abrir un chat; integra apps con la API REST en localhost:11434; y crea modelos personalizados con Modelfiles.',
    h2QuickAnswers: 'Respuestas rapidas sobre comandos de Ollama',
    quickFineTuneQ: 'Ollama soporta fine-tuning?',
    quickFineTuneA: 'Ollama no entrena ni ajusta modelos directamente. El flujo practico es hacer fine-tuning con herramientas como Unsloth, Axolotl o llama.cpp, exportar un modelo GGUF o adapter y despues importarlo en Ollama con un Modelfile usando FROM o ADAPTER y ollama create.',
    quickRunQ: 'Como ejecuto un modelo con Ollama?',
    quickRunA: 'Usa ollama run nombre-del-modelo. Por ejemplo, ollama run llama3 descarga el modelo si hace falta y abre un chat interactivo. Para automatizar, ejecuta primero ollama pull nombre-del-modelo y despues llama a la API REST en localhost:11434.',
    quickPullQ: 'Cual es el comando oficial para descargar modelos?',
    quickPullA: 'Usa ollama pull nombre-del-modelo para descargar o actualizar un modelo sin iniciar una sesion de chat. Usa ollama list para ver modelos descargados, ollama show nombre-del-modelo para detalles y ollama rm nombre-del-modelo para eliminarlo.',
    keyTakeaways: 'Puntos clave',
    kt1: 'Ollama soporta mas de 100 modelos, incluidos Llama 3, Mistral, Code Llama, Phi-3 y Gemma 2',
    kt2: 'La instalacion es sencilla en macOS y Linux; Docker funciona en todas las plataformas',
    kt3: 'La API REST en localhost:11434 ofrece endpoints como /api/generate, /api/chat y /api/embeddings',
    kt4: 'Los Modelfiles permiten ajustar parametros, prompts de sistema y modelos especializados',
    kt5: 'La aceleracion GPU con CUDA, Metal y ROCm mejora mucho el rendimiento',
    kt6: 'Los modelos 7B suelen necesitar 8 GB de RAM; 13B requiere 16 GB y 70B al menos 64 GB',
    h2WhatIs: 'Que es Ollama y por que ejecutar LLMs localmente?',
    whatIsP1: 'Ollama es un framework ligero para ejecutar modelos de lenguaje en tu maquina local. Envuelve llama.cpp con una CLI y una API REST faciles de usar, gestionando descargas, cuantizacion, GPU y memoria.',
    whatIsP2: 'Ejecutar LLMs localmente ofrece tres ventajas: tus prompts no salen de tu equipo, no pagas por token y reduces la latencia al evitar viajes de red.',
    whatIsP3: 'Para desarrolladores, Ollama es una forma practica de probar modelos, crear asistentes internos, construir RAG local y desplegar inferencia privada sin depender de un proveedor cloud.',
    h2Install: 'Guia de instalacion',
    h3MacOS: 'macOS (Intel y Apple Silicon)',
    macDesc: 'Ollama tiene soporte de primera clase en macOS y usa Metal automaticamente en Apple Silicon. La instalacion suele tardar menos de un minuto.',
    h3Linux: 'Linux',
    linuxDesc: 'En Linux, el script oficial instala Ollama y detecta CUDA cuando hay GPU NVIDIA. Tambien puede ejecutarse como servicio systemd.',
    h3Windows: 'Windows',
    windowsDesc: 'Ollama incluye instalador nativo para Windows con soporte GPU. WSL2 ya no es obligatorio para el uso normal.',
    h3Docker: 'Docker (todas las plataformas)',
    dockerInstallDesc: 'Docker es la opcion mas portable para macOS, Linux y Windows. En Linux puede usar GPU NVIDIA con NVIDIA Container Toolkit.',
    h2Models: 'Ejecutar modelos',
    modelsDesc: 'La libreria de modelos de Ollama incluye opciones listas para usar. El comando ollama run descarga el modelo si falta y abre una sesion interactiva.',
    h3PopularModels: 'Modelos populares',
    h3ModelVariants: 'Comparacion de rendimiento de modelos',
    variantsDesc: 'Los modelos pequenos son rapidos y consumen menos memoria; los grandes producen mejores respuestas pero requieren mas RAM o VRAM.',
    h2Management: 'Gestion de modelos',
    managementDesc: 'Ollama permite listar, descargar, eliminar e inspeccionar modelos locales para mantener el entorno limpio y ahorrar espacio en disco.',
    h2Api: 'API REST de Ollama',
    apiDesc: 'Ollama expone una API REST en localhost:11434 para integrar modelos locales en aplicaciones. Los endpoints soportan streaming por defecto y pueden sustituir muchos flujos compatibles con OpenAI.',
    h3Generate: '/api/generate — generacion de texto',
    h3Chat: '/api/chat — chat conversacional',
    h3Embeddings: '/api/embeddings — embeddings vectoriales',
    embeddingsDesc: 'Los embeddings convierten texto en vectores numericos para busqueda semantica, RAG y comparacion de documentos.',
    h2Modelfile: 'Crear Modelfiles personalizados',
    modelfileDesc: 'Un Modelfile funciona como un Dockerfile para LLMs: define el modelo base, parametros, prompt de sistema y plantilla.',
    h3Directives: 'Directivas de Modelfile',
    directivesDesc: 'Las directivas clave son FROM, PARAMETER, SYSTEM, TEMPLATE y ADAPTER para aplicar pesos LoRA o importar modelos afinados.',
    h2Faq: 'Preguntas frecuentes',
    faq1Q: 'Que hardware necesito para ejecutar Ollama?',
    faq1A: 'Para modelos 7B, lo practico es tener al menos 8 GB de RAM. Un Mac Apple Silicon con 16 GB o una GPU NVIDIA con 8 GB de VRAM funciona muy bien. Para modelos 70B, necesitas mucha mas memoria, normalmente 64 GB o mas.',
    faq2Q: 'Ollama es gratis?',
    faq2A: 'Si. Ollama es gratis y open-source bajo licencia MIT. No hay cuotas por uso ni costes por token.',
    faq3Q: 'Como se compara Ollama con ChatGPT?',
    faq3A: 'Ollama ejecuta modelos en tu equipo; ChatGPT usa servidores externos. Los modelos locales suelen ser menos potentes que GPT-4, pero ofrecen privacidad, coste cero por consulta y control total.',
    faq4Q: 'Puedo usar Ollama para programacion?',
    faq4A: 'Si. Modelos como Code Llama, DeepSeek Coder y StarCoder2 sirven para explicar codigo, depurar, generar funciones y revisar cambios.',
    faq5Q: 'Ollama soporta fine-tuning?',
    faq5A: 'Ollama no hace fine-tuning directamente, pero puede importar modelos GGUF o adapters creados con otras herramientas y ejecutarlos con Modelfiles.',
    faq6Q: 'Ollama puede ejecutar varios modelos a la vez?',
    faq6A: 'Si, siempre que tengas suficiente RAM o VRAM. Usa keep_alive para controlar cuanto tiempo quedan cargados los modelos.',
    faq7Q: 'Como actualizo Ollama y los modelos?',
    faq7A: 'Actualiza Ollama desde ollama.com o con el instalador correspondiente. Para actualizar modelos, usa ollama pull nombre-del-modelo.',
    faq8Q: 'Mis datos son privados con Ollama?',
    faq8A: 'Si. La inferencia se ejecuta localmente y tus prompts no se envian a servidores externos, lo que lo hace util para codigo privado y documentos sensibles.',
  },
};
