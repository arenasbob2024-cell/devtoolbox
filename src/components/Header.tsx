...
export default function Header() {
  ...
  const results = search.length > 0
    ? tools.filter(tool => {
        ...
        const name = toolDict?.name || tool.name;
            ...
            <div style={{
              ...
              {results.map(tool => {
                ...
                return (
                  <div
                    key={tool.id}
                    onClick={() => { router.push(`/${lang}${tool.path}`); setSearch(''); setShowResults(false); }}
                    style={{
                      padding: '10px 14px',
                      cursor: 'pointer',
                      display: 'flex',
                  ...
                  >
                    <span style={{ fontSize: 16, width: 32, textAlign: 'center', fontFamily: 'monospace', fontWeight: 700 }}>{tool.icon}</span>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600 }}>{toolDict?.name || tool.name}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{toolDict?.description || tool.description}</div>
                    </div>
                  </div>
                );
              })}
