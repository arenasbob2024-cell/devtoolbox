...
Long-scroll discovery pages now include a restrained mid-list Adsterra opportunity. The homepage tools grid, all-tools index, category tool grids, and blog listing insert one mid-scroll leaderboard only after enough organic content has appeared. These slots reuse the relevant existing placement envs (`HOME_INLINE`, `TOOLS_INDEX_BOTTOM`, `CATEGORY_BOTTOM`, or `BLOG_BOTTOM`) and otherwise fall back to the global top leaderboard key, so they add viewable inventory immediately without creating another required Adsterra dashboard setup step. Track them by placements such as `home-tools-grid`, `tools-index-mid`, `category-mid`, and `blog-list-mid`; if one wins on RPM, create a dedicated unit later and split it out.
...
Mobile tool pages now also show a post-action 300x250 rectangle with `placement=tool-mobile-rectangle`, inserted after the share bar and before comments. It only mounts on mobile viewports, so desktop does not create hidden ad requests, and it reuses `NEXT_PUBLIC_ADSTERRA_SIDEBAR_KEY` until a dedicated mobile rectangle unit is worth splitting out.
