...
Bottom Adsterra slots now fall back to a direct sponsorship CTA when the corresponding Adsterra key is not configured. This applies to tool pages, tools index, blog list, blog articles, and category pages, using placements such as `tool-bottom-ad-fallback` and `blog-article-bottom-ad-fallback` so empty ad inventory still becomes a measurable sponsor opportunity.
